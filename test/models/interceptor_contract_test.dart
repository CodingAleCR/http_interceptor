  ...
  group('InterceptorContract', () {
    group('TestInterceptor', () {
      test('should implement all required methods', () {
        ...
        expect(interceptor.interceptRequest(request: request),
            isA<FutureOr<BaseRequest>>());
        expect(interceptor.interceptResponse(response: response),
            isA<FutureOr<BaseResponse>>());
      });
    ...
    group('ModifyingInterceptor', () {
      test('should add headers to request', () async {
        ...
        expect(
            modifiedRequest.headers['Authorization'], equals('Bearer token'));
        expect(modifiedRequest.headers['Content-Type'],
      ...
      test(
          'should return true for shouldInterceptResponse when bodyPrefix is provided',
          () async {
      ...
      test('should return false for shouldInterceptResponse when no bodyPrefix',
          () async {
        final interceptor = ModifyingInterceptor();
