  ...
  group('InterceptedClient', () {
    ...
    group('Basic HTTP Methods', () {
      ...
      test('should perform POST request with interceptors', () async {
        ...
        expect(
            interceptor.log, contains('interceptRequest: POST $baseUrl/test'));
    ...
    group('Request Headers', () {
      ...
      test('should handle multiple values for same header', () async {
        ...
        final headers = responseData['headers'] as Map<String, dynamic>;
        expect(
            headers['accept'], contains('application/json, text/plain, */*'));
    ...
    group('Query Parameters', () {
      ...
      test('should handle special characters in query parameters', () async {
        ...
        expect(response.statusCode, equals(200));
        final responseData = jsonDecode(response.body) as Map<String, dynamic>;
        expect(responseData['url'],
            contains('param+with+spaces=value+with+spaces'));
        expect(responseData['url'],
            contains('param%26with%3Dspecial=value%26with%3Dspecial'));
        expect(responseData['url'],
            contains('param%2Bwith%2Bplus=value%2Bwith%2Bplus'));
    ...
    group('Response Handling', () {
      ...
      test('should handle response with custom status code', () async {
        final interceptor =
            ResponseModifierInterceptor(statusCode: 201, body: 'created');
        final client = InterceptedClient.build(interceptors: [interceptor]);
    ...
    group('Streamed Responses', () {
      ...
      test('should handle StreamedResponse with different status codes',
          () async {
        final interceptor = TestInterceptor();
        final client = InterceptedClient.build(interceptors: [interceptor]);
    ...
    group('Interceptor Chaining', () {
      ...
      test('should handle request modification by interceptor', () async {
        ...
        final interceptor =
            TestInterceptor(requestModification: modifiedRequest);
        final client = InterceptedClient.build(interceptors: [interceptor]);
    ...
    group('Request Types and Edge Cases', () {
      ...
      test('should handle Request with different HTTP methods', () async {
        ...
        for (final method in methods) {
          ...
          expect(response.statusCode, equals(200));
          expect(interceptor.log,
              contains('shouldInterceptRequest: $method $baseUrl/test'));
        }
      ...
      test('should handle Request with special characters in URL', () async {
        ...
        final uri = Uri.parse(
            '$baseUrl/test/path with spaces/param?key=value with spaces');
        final request = Request('GET', uri);
        ...
        expect(
            interceptor.log,
            contains(
                'shouldInterceptRequest: GET $baseUrl/test/path%20with%20spaces/param?key=value%20with%20spaces'));
    ...
    group('Error Handling', () {
      ...
      test('should handle timeout errors', () async {
        ...
        expect(
          () => client.get(Uri.parse('http://10.255.255.1'),
    ...
    group('Multipart Requests', () {
      ...
      test('should handle multipart request with special characters in fields',
          () async {
        final interceptor = TestInterceptor();
        final client = InterceptedClient.build(interceptors: [interceptor]);
      ...
      test('should handle multipart request with files and no fields',
          () async {
        final interceptor = TestInterceptor();
        final client = InterceptedClient.build(interceptors: [interceptor]);
      ...
      test('should handle multipart request with fields and no files',
          () async {
        final interceptor = TestInterceptor();
        final client = InterceptedClient.build(interceptors: [interceptor]);
      ...
      test('should handle multipart request with empty fields and files',
          () async {
        final interceptor = TestInterceptor();
        final client = InterceptedClient.build(interceptors: [interceptor]);
      ...
      test('should handle multipart request with different content types',
          () async {
        final interceptor = TestInterceptor();
        final client = InterceptedClient.build(interceptors: [interceptor]);
      ...
      test('should handle multipart request with interceptor modification',
          () async {
        final modifiedRequest =
            MultipartRequest('PUT', Uri.parse('$baseUrl/modified'));
      ...
      test('should handle multipart request with conditional interception',
          () async {
        final interceptor = TestInterceptor(
          shouldInterceptRequest: false,
          shouldInterceptResponse: false,
    ...
    group('Complex Scenarios', () {
      test('should handle complex request with all features', () async {
        ...
        expect(
            interceptor.log,
            contains(
                'shouldInterceptRequest: POST $baseUrl/test?query1=value1&query2=value2'));
