  ...
  group('InterceptedClient', () {
    ...
    group('HTTP Methods', () {
      ...
      test('should call post method', () async {
        ...
        verify(mockClient.post(uri, 
            headers: anyNamed('headers'), 
            body: body,
            encoding: anyNamed('encoding'))).called(1);
      });
    ...
    group('Interceptor Integration', () {
      test('should call interceptor shouldInterceptRequest', () async {
        ...
        when(mockInterceptor.interceptRequest(request: anyNamed('request')))
            .thenAnswer((_) async => http.Request('GET', Uri.parse('https://example.com')));
        ...
        verify(mockInterceptor.interceptRequest(request: anyNamed('request'))).called(1);
      });
      ...
      test('should call interceptor shouldInterceptResponse', () async {
        ...
        when(mockInterceptor.interceptResponse(response: anyNamed('response')))
            .thenAnswer((_) async => http.Response('intercepted response', 200));
        ...
        verify(mockInterceptor.interceptResponse(response: anyNamed('response'))).called(1);
      });

      test('should skip interceptor when shouldInterceptRequest returns false', () async {
        when(mockInterceptor.shouldInterceptRequest())
            .thenAnswer((_) async => false);
        when(mockInterceptor.shouldInterceptResponse())
            .thenAnswer((_) async => false);
        ...
        verify(mockInterceptor.shouldInterceptRequest()).called(1);
        verifyNever(mockInterceptor.interceptRequest(request: anyNamed('request')));
      });

      test('should skip interceptor when shouldInterceptResponse returns false', () async {
        when(mockInterceptor.shouldInterceptRequest())
            .thenAnswer((_) async => false);
        when(mockInterceptor.shouldInterceptResponse())
            .thenAnswer((_) async => false);
        ...
        verify(mockInterceptor.shouldInterceptResponse()).called(1);
        verifyNever(mockInterceptor.interceptResponse(response: anyNamed('response')));
      });
    });
    ...
    group('Retry Policy Integration', () {
      ...
      test('should respect max retry attempts', () async {
        ...
        expect(() => client.get(uri), throwsException);
        verify(mockClient.get(uri, headers: anyNamed('headers'))).called(2); // Original + 1 retry
      });
    });
    ...
    group('Error Handling', () {
      ...
      test('should handle response interceptor exceptions gracefully', () async {
        when(mockInterceptor.shouldInterceptRequest())
            .thenAnswer((_) async => false);
        when(mockInterceptor.shouldInterceptResponse())
            .thenAnswer((_) async => true);
    ...
    group('Complex Scenarios', () {
      test('should handle multiple interceptors in order', () async {
        ...
        when(interceptor1.shouldInterceptRequest()).thenAnswer((_) async => true);
        when(interceptor1.interceptRequest(request: anyNamed('request')))
            .thenAnswer((invocation) async {
          final request = invocation.namedArguments[#request] as http.BaseRequest;
          request.headers['X-Interceptor-1'] = 'true';
          return request;
        });
        when(interceptor1.shouldInterceptResponse()).thenAnswer((_) async => false);
        
        when(interceptor2.shouldInterceptRequest()).thenAnswer((_) async => true);
        ...
        when(interceptor2.shouldInterceptResponse()).thenAnswer((_) async => false);
        ...
        verify(interceptor1.interceptRequest(request: anyNamed('request'))).called(1);
        verify(interceptor2.interceptRequest(request: anyNamed('request'))).called(1);
      });

      test('should handle interceptors with retry policy', () async {
        when(mockInterceptor.shouldInterceptRequest()).thenAnswer((_) async => true);
        when(mockInterceptor.interceptRequest(request: anyNamed('request')))
            .thenAnswer((invocation) async => invocation.namedArguments[#request] as http.BaseRequest);
        when(mockInterceptor.shouldInterceptResponse()).thenAnswer((_) async => false);
        ...
        expect(response.body, equals('success'));
        verify(mockInterceptor.interceptRequest(request: anyNamed('request'))).called(2);
      });
    });
  });
}